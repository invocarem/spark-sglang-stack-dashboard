...
import {
  ...
  streamModelTransferInContainer,
...
export function registerCoreRoutes(app: Hono): void {
  ...
  app.post("/api/model-transfer/stream", async (c) => {
    let body: unknown;
    try {
      body = await c.req.json();
    } catch {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          await streamModelTransferInContainer(
            container,
            {
              role,
              modelDir,
  ...
  app.post("/api/model-download/stream", async (c) => {
    let body: unknown;
    try {
      body = await c.req.json();
    } catch {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          await streamModelDownloadInContainer(
            container,
            { modelId, saveDir },
            { timeoutMs: cappedTimeout, maxOutputBytes: 5_000_000 },
            (ev) => {
  ...
  app.post("/api/benchmark-sglang/stream", async (c) => {
    let body: unknown;
    try {
      body = await c.req.json();
    } catch {
    ...
    const parsed = parseBenchmarkSglangRequest(body as Record<string, unknown>);
    if (!parsed.ok) {
    ...
    const toolLog = createMonitorToolLogSession("benchmark-sglang", {
      container,
      baseUrl: params.baseUrl,
      numPrompts: params.numPrompts,
      timeoutMs,
    });
    if (toolLog) {
      c.header("X-Monitor-Tool-Log", toolLog.relativePath);
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          await streamBenchmarkSglangInContainer(
            container,
            params,
